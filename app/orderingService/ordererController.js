...
class OrdererController {
    ...
    static async enrollOrderingNodes(ordererRequest) {
        ...
        let admnSecret = savedOrderingService.caId.admnSecret;
        let namespace = savedOrderingService.caId.networkId.namespace;
        let caPort = savedOrderingService.caId.port;
        let rootCaname = savedOrderingService.caId.name;
        ...
        if (ordererRequest.isTLS) {
            ...
            admnSecret = savedOrderingService.tlsCaId.admnSecret;
            namespace = savedOrderingService.tlsCaId.networkId.namespace;
            caPort = savedOrderingService.tlsCaId.port;
        }
        ...
        const basePath = `${utils.getCaBasePath(namespace, savedcaName)}`; //network1/root-ca
        const ccpPath = `${basePath}/config.json`;
        const tlsPath = `${basePath}/tls-cert.pem`;
        const ccpJSON = fs.readFileSync(ccpPath, "utf8");
        const ccp = JSON.parse(ccpJSON);
        ...
        for (let i = 0; i < orderersList.length; i++) {
            ...
            console.log("orderer fabric client path ", fabric_ca_client_home);
            // const basePath =
            let orderer_enroll_id = orderer.name;
            let orderer_enroll_secret = admnSecret;
            let tls_ca_port = caPort;
            let ca_name = savedcaName;
            let orderer_tls_path = `${fabric_ca_client_home}/orderers/${orderer.name}/orderer-tls`;
            let csr_host1 = orderer.name;
            let csr_host2 = "localhost";
            let url = `https://${orderer_enroll_id}:${orderer_enroll_secret}@localhost:${tls_ca_port}`;

            const path = `${os.homedir}/fabric-samples/bin:$PATH`;

            console.log("url ", url);
            console.log("orderermsp path ", orderer_tls_path);

            let whole_command = `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${orderer_tls_path} --enrollment.profile tls --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --tls.certfiles ${tlsPath}`;
            ...
            if (
                shell.exec(
                `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${orderer_tls_path} --enrollment.profile tls --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --tls.certfiles ${tlsPath}`
                ).code !== 0
            ) {
                shell.echo("Error: while creating genesis block");
                throw new exceptions.EnrollException(
                `Error: while enrolling  ${orderer.name}`
                );
            }
            console.log("@@@@@@@@@@@@ check 2 enrolled successfully");

            // ??????
            const ordererTlscCaCertspPath = `${orderer_tls_path}/tlscacerts/`;

            console.log("orderer tls path ", ordererTlscCaCertspPath);

            const ordererKeystorePath = `${orderer_tls_path}/keystore`;

            // ?????
            const ordererSingcertsPath = `${orderer_tls_path}/signcerts`;

            orderer.admnSecret = admnSecret;
            orderer.tlsCacerts = await readFile(ordererTlscCaCertspPath);
            orderer.tlsPrimaryKey = await readFile(ordererKeystorePath);
            orderer.tlsSignCert = await readFile(ordererSingcertsPath);

            console.log("orderres ", orderer);
            } else {
            ...
            let tls_ca_port = caPort;
            let ca_name = savedcaName;
            let orderer_msp_path = `${fabric_ca_client_home}/orderers/${orderer.name}/orderer-msp`;
            let csr_host1 = orderer.name;
            let csr_host2 = "localhost";
            let url = `https://${orderer_enroll_id}:${orderer_enroll_secret}@localhost:${tls_ca_port}`;
            ...
            console.log("url ", url);
            console.log("orderermsp path ", orderer_msp_path);

            let whole_command = `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${orderer_msp_path} --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --tls.certfiles ${tlsPath}`;
            ...
            if (
                shell.exec(
                `export PATH=${path} && export FABRIC_CA_CLIENT_HOME=${fabric_ca_client_home} && fabric-ca-client enroll -u ${url} --caname ${ca_name} -M ${orderer_msp_path} --csr.hosts ${csr_host1} --csr.hosts ${csr_host2} --tls.certfiles ${tlsPath}`
                ).code !== 0
            ) {
                shell.echo("Error: while creating genesis block");
            ...
            // ??????
            const ordererMSPPath = `${orderer_msp_path}/cacerts/`;

            const ordererKeystorePath = `${orderer_msp_path}/keystore`;

            // ?????
            const ordererSingcertsPath = `${orderer_msp_path}/signcerts`;

            orderer.admnSecret = admnSecret;
            orderer.cacets = await readFile(ordererMSPPath);
            orderer.primaryKey = await readFile(ordererKeystorePath);
            orderer.signCert = await readFile(ordererSingcertsPath);
    ...
    static async generateOrderingServiceMsp(ordererMspRequest) {
        ...
        console.log("ordererList ", orderersList);
        ...
        for (let i = 0; i < orderersList.length; i++) {
            ...
            fs.outputFileSync(`${orgMspPath}/cacerts/ca.pem`, ordererNode.cacets);
            fs.outputFileSync(
            ...
            fs.outputFileSync(
            `${tlsMspPath}/tlscacerts/tlsca.pem`,
            ordererNode.tlsCacerts
            );
            fs.outputFileSync(
            ...
            fs.outputFileSync(
            `${tlsMspPath}/keystore/key.pem`,
            ordererNode.tlsPrimaryKey
            );
            fs.outputFileSync(`${tlsMspPath}/server.crt`, ordererNode.tlsSignCert);
            fs.outputFileSync(
            `${tlsMspPath}/signcerts/certs.pem`,
            ...
            logger.debug("%s - orderer %s MSP generated", method, ordererNode.name);
        }
    ...
    static async createGenesisBlock(genesisRequest) {
        ...
        try {
            ...
            const genesisFilePath = `${basePath}/channel-artifacts/`;
