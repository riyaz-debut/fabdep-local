...
# echo '{"payload":{"header":{"channel_header":{"channel_id":"'${channelName}'", "type":2}},"data":{"config_update":'$(cat ${newOrg_update_json_path})'}}}' | jq . > ${newOrg_update_envelope_path} && configtxlator proto_encode --input ${newOrg_update_envelope_json_path} --type common.Envelope --output ${newOrg_update_envelope_pb_path}
